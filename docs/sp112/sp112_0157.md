...
输入参数*rtol*和*atol*确定求解器执行的误差控制。求解器将根据形式为`max-norm of (e / ewt) <= 1`的不等式控制y的估计局部误差向量e，其中ewt是计算为`ewt = rtol * abs(y) + atol`的正误差权重向量。rtol和atol可以是与y相同长度的向量或标量。默认为1.49012e-8。
