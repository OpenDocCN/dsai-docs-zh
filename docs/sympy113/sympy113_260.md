...
**use_unicode_sqrt_char**：bool，可选（默认为True）
...
此类使用几个规则来决定使用哪个符号来表示乘积。纯数字使用“*”，符号使用“.*”，MatrixSymbols使用“*”。HadamardProduct可用于指定两个MatrixSymbols的逐分量乘法“.*”。目前还没有简单的方法来指定标量符号，因此有时代码可能会有些轻微的外观问题。例如，假设x和y是标量，A是矩阵，那么虽然人类程序员可能会写成“(x^2*y)*A^3”，我们生成的代码是：
...
参见[自定义打印方法示例](#printer-method-example)，了解如何通过实现 `_latex` 来覆盖自己类型的此行为的示例。

从版本1.7.0开始更改：不再将不支持的类型的`str`表示形式视为有效的latex。
...
关于mathml的更多信息：[https://www.w3.org/TR/MathML2](https://www.w3.org/TR/MathML2)
...
参考资料：[https://www.w3.org/TR/MathML2/chapter4.html](https://www.w3.org/TR/MathML2/chapter4.html)
...
参考资料：[https://www.w3.org/TR/MathML2/chapter3.html](https://www.w3.org/TR/MathML2/chapter3.html)
...
[`print_tree`](#sympy.printing.tree.print_tree "sympy.printing.tree.print_tree")
...
[`tree`](#sympy.printing.tree.tree "sympy.printing.tree.tree")
...
附加关键字参数将传递给 [`latex()`](#sympy.printing.latex.latex "sympy.printing.latex.latex") 调用，例如 `symbol_names` 标志。
...
请查看pretty.py函数_print_meijerg，_print_hyper，了解如何使用此信息。

由Jurjen Bos编写的Prettyprinter。（我讨厌垃圾邮件：请发邮件至ku.oc.oohay的反向地址）。所有对象都有一个创建“stringPict”的方法，可在用于漂亮打印的str方法中使用。

由Jason Gedge更新（电子邮件<我的姓氏> at cs mun ca）

+   terminal_string() 方法

+   小修复和更改（主要是对prettyForm的修复）
...
将图片放在此图片上方。返回字符串，用于stringPict的基线参数。基线是底部图片的基线。

```py
below(*args)
```

将图片放在此图片下方。返回字符串，用于stringPict的基线参数。基线是顶部图片的基线。
...
将图片（从左到右）放在左边。返回字符串，用于stringPict的基线参数。
...
将一串stringPicts放在一起。返回字符串，用于stringPict的基线参数。

```py
parens(left='(', right=')', ifascii_nougly=False)
```

将自身放在括号中。返回字符串，用于stringPict的基线参数。

左侧或右侧可以是None或空字符串，表示“该侧没有括号”。
...
除非设置参数`line_break`为False，否则将在可以在终端上打印的形式中断表达式，而不会中断它。

```py
right(*args)
```

将图片放在这个旁边。返回字符串，用于stringPict的基线参数。（多行）字符串被允许，并且给出基线为0。
...
生成一个漂亮的根符号。对大n插入产生丑陋的结果。

```py
static stack(*args)
```

将图片从上到下放在一起。返回字符串，用于stringPict的基线参数。基线是第二张图片的基线。一切都居中。基线是第二张图片的基线。允许字符串。特殊值stringPict.LINE是一排‘-’扩展到宽度。
...
stringPict类的扩展，了解基本数学应用程序，优化双减号。
