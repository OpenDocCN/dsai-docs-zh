...
> 原始文本：[https://docs.scipy.org/doc/scipy-1.12.0/reference/generated/scipy.optimize.line_search.html#scipy.optimize.line_search](https://docs.scipy.org/doc/scipy-1.12.0/reference/generated/scipy.optimize.line_search.html#scipy.optimize.line_search)
...
Alpha 使得 `x_new = x0 + alpha * pk`，如果线搜索算法未收敛，则为None。
...
新函数值 `f(x_new)=f(x0+alpha*pk)`，如果线搜索算法未收敛，则为None。

**old_fval**float

旧函数值 `f(x0)`。

**new_slope**float或None

在新值处沿搜索方向的局部斜率 `<myfprime(x_new), pk>`，如果线搜索算法未收敛，则为None。

注意事项

使用线搜索算法来强制实施强 Wolfe 条件。参见 Wright 和 Nocedal，《Numerical Optimization》，1999 年，第 59-61 页。

搜索方向 *pk* 必须是下降方向（例如 `-myfprime(xk)`）以找到满足强 Wolfe 条件的步长。如果搜索方向不是下降方向（例如 `myfprime(xk)`），则 *alpha*、*new_fval* 和 *new_slope* 将为None。
