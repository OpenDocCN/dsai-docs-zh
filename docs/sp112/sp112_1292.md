...
> 原文链接：[https://docs.scipy.org/doc/scipy-1.12.0/reference/generated/scipy.stats.quantile_test.html#scipy.stats.quantile_test](https://docs.scipy.org/doc/scipy-1.12.0/reference/generated/scipy.stats.quantile_test.html#scipy.stats.quantile_test)
...
置信区间的方法归因于Thompson [[2]](#r583b5b824ca0-2)，并且后来被证明适用于任何一组独立同分布样本 [[3]](#r583b5b824ca0-3)。计算基于这样的观察：分位数 \(q\) 大于任何观测值 \(x_m (1\leq m \leq N)\) 的概率可以计算为
...
计算分位数置信区间的相同方法包含在confintr包中[[5]](#r583b5b824ca0-5)。
...
1.  1.  Conover，《实用非参数统计学》，第3版，1999年。

[[2](#id2)]

W. R. Thompson，《关于中位数和其他期望分布的置信区间》，《数理统计学年刊》，卷7，第3期，pp. 122-128，1936年，访问日期：2019年9月18日。[在线]. 可用：[https://www.jstor.org/stable/2957563](https://www.jstor.org/stable/2957563).

[[3](#id3)]

H. A. David 和 H. N. Nagaraja，《非参数推断中的序统计量》，《序统计量》，John Wiley & Sons, Ltd，2005年，pp. 159-170. 可用：[https://onlinelibrary.wiley.com/doi/10.1002/0471722162.ch7](https://onlinelibrary.wiley.com/doi/10.1002/0471722162.ch7).

[[4](#id4)]

N. Hutson, A. Hutson, L. Yan，《QuantileNPCI: 非参数置信区间的分位数》，R 包，[https://cran.r-project.org/package=QuantileNPCI](https://cran.r-project.org/package=QuantileNPCI)

[[5](#id5)]

M. Mayer，《confintr: 置信区间》，R 包，[https://cran.r-project.org/package=confintr](https://cran.r-project.org/package=confintr)

例子

假设我们希望测试一个总体中位数等于0.5的零假设。我们选择99%的置信水平；也就是说，如果p值小于0.01，我们将拒绝零假设，接受备择假设。

当测试来自标准均匀分布的随机变量时，其中位数为0.5，我们预期数据大部分时间与零假设一致。
...
顾名思义，p值未低于我们的0.01阈值，因此我们无法拒绝零假设。

当测试来自标准*正态*分布的数据时，其中位数为0，我们预期会拒绝零假设。
...
实际上，p值低于我们的0.01阈值，因此我们拒绝零假设，接受默认的“双边”备择假设：总体中位数*不*等于0.5。

然而，假设我们要测试一个单侧备择假设，即总体中位数*大于*0.5。由于标准正态分布的中位数小于0.5，我们不希望拒绝零假设。
...
不出所料，由于p值高于我们的阈值，我们不会拒绝零假设，而是接受所选择的备择假设。

分位数测试不仅可以用于中位数，还可以用于任何分位数。例如，我们可以测试样本基础分布的第三四分位数是否大于0.6。
...
p值低于阈值。我们拒绝零假设，接受备择假设：样本基础分布的第三四分位数大于0.6。

[`quantile_test`](#scipy.stats.quantile_test "scipy.stats.quantile_test") 还可以计算任何分位数的置信区间。
...
在测试单侧备择假设时，置信区间包含所有观察结果，使得如果作为*q*，则测试的p值大于0.05，因此不会拒绝原假设。例如：
...
此外，如果针对随机样本重复生成95%置信区间，则在大约95%的复制中，置信区间将包含真实的分位值。
